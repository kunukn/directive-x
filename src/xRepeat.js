...
module.exports = function(t, path, attrs) {
  var openingEl = path.node.openingElement,
    ...
    props = openingEl.attributes.filter(prop => prop.name.name !== 'xRepeat'),
    children = path.node.children,
    directive =
      openingEl.attributes[attrs.findIndex(attr => attr === 'xRepeat')],
    {
      value: {
        expression: {
    ...
    toRepIdentifierComputed = t.isStringLiteral(toRepIdentifier.property),
  ...
  return t.callExpression(
    ...
    [
      t.arrowFunctionExpression(
        [t.identifier(toRepAsIdentifier), t.identifier('i')],
        t.blockStatement([t.returnStatement(returnStatement)])
      ),
    ]
  );
};
