...
module.exports = function(t, path, attrs) {
  ...
  if (!attrs.length || !attrs.includes('xBind')) return;
  ...
  var compIdentifier = t.JSXIdentifier(openingEl.name.name),
    ...
    directive = openingEl.attributes[attrs.findIndex(attr => attr === 'xBind')],
    bindTo =
      directive.value && directive.value.expression.type !== 'ThisExpression'
    ...
    constructorMethodKey = classMethods
      .map((cm, i) => cm.kind === 'constructor' && i)
      .filter(i => i !== false)[0],
    constructor =
      constructorMethodKey >= 0
    ...
    xBindEvenHandlersClassMethodKey = classMethods
      .map((cm, i) => cm.key.name === 'xBindEvenHandlers' && i)
      .filter(i => i !== false)[0],
    xBindEvenHandlersClassMethod =
      classDeclaration.node.body.body[xBindEvenHandlersClassMethodKey],
    xBindsCalledInConstructor = constructor.body.body
      .map(
        v =>
          v.expression &&
          v.expression.callee.property &&
          v.expression.callee.property.name === 'xBindEvenHandlers'
      )
      .filter(v => v)[0],
    ...
    eventHandlerIDs =
      JSXOpeningElementSrc &&
      JSXOpeningElementSrc.map(str => {
        return t.identifier(
          str.replace(/on\w+\s*\=\s*\{/, '').replace(/this\./, '')
        );
      });
  ...
  if (!xBindEvenHandlersClassMethod) {
    ...
    classDeclaration
      .get('body')
      .unshiftContainer('body', xBindEvenHandlersClassMethod);
  }

  // safely extend xBindEvenHandlers class method
  for (let i = 0; i < eventHandlerIDs.length; i++) {
    xBindEvenHandlersClassMethod.body.body[
      xBindEvenHandlersClassMethod.body.body.length + i
